...
def train_and_eval(dataset: str, seed: int, filter_name: str, filter_fn, *, noise=0.1, M=1024, L=64, M_S=8, alpha=0.3, epochs=60):
    set_seed(seed)
...
def summarize(results):
    ...
    for dataset in sorted(set(r['dataset'] for r in results)):
        for filter_name in sorted(set(r['filter'] for r in results)):
            rows = [r for r in results if r['dataset'] == dataset and r['filter'] == filter_name]
            key = f'{dataset}::{filter_name}'
            summary[key] = {}
            for metric in ['rmse16', 'rmse32', 'rmse48', 'final_loss']:
    ...
    for dataset in ['pendulum', 'lorenz']:
        for seed in [0, 1, 2, 3, 4]:
            for name, fn in filters.items():
                print(f'RUN dataset={dataset} seed={seed} filter={name}', flush=True)
                results.append(train_and_eval(dataset, seed, name, fn))
    payload = {'runs': results, 'summary': summarize(results)}
    out_path = out_dir / 'tvnn_fourier_extended.json'
